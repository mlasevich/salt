    ...
    def setUpClass(cls):  # pylint: disable=arguments-differ
        ...
        try:
            ...
            if cls.sshd_proc is not None:
                if cls.sshd_proc.is_running():
                    cls.sshd_proc.terminate()
                    cls.sshd_proc = None
                elif not psutil.pid_exists(cls.sshd_proc.pid):
                    log.info(
    ...
    def setUpClass(cls):  # pylint: disable=arguments-differ
        ...
        try:
            ...
            if cls.uwsgi_proc is not None:
                if cls.uwsgi_proc.is_running():
                    cls.uwsgi_proc.terminate()
                    cls.uwsgi_proc = None
                elif not psutil.pid_exists(cls.uwsgi_proc.pid):
                    log.warning(
            ...
            if cls.nginx_proc is not None:
                if cls.nginx_proc.is_running():
                    cls.nginx_proc.terminate()
                    cls.nginx_proc = None
                elif not psutil.pid_exists(cls.nginx_proc.pid):
                    log.warning(
